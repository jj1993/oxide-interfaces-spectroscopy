...
smooth = .0003
FILE_LCflip = "../../Data/i09-75691.nxs"
FILE_LC = "../../Data/i09-75711.nxs"
FILE_RCflip = "../../Data/i09-75692.nxs"
FILE_RC = "../../Data/i09-75712.nxs"
FILE_LC_RT = "../../Data/i09-75678.nxs"
FILE_RC_RT = "../../Data/i09-75679.nxs"
...
def getBackground(energies, spectrum):
    ...
    points = [1, 35, 80, 155, 190, 245, 278]
    B = 1
    answ = "n"
...
##    m_o, m_o_err, m_s, m_s_err = getMagnetism(RCSpectrum_norm, LCSpectrum_norm, energies, reg)
    ...
    XMCD_RT = [(LCSpectrum_RT_norm[n]-s)/max(LCSpectrum_RT_norm) for n, s in enumerate(RCSpectrum_RT_norm)]
...
##    plt.plot(RCEnergies, LCNorm)
    plt.plot(energies, LCSpectrum_norm, linewidth=2.0, label='$\sigma^+H^+$ LT')
##    plt.plot(RCEnergies, LCflipSpectrum_norm, 'k--',label='$\sigma^+H^-$')
    plt.plot(energies, RCSpectrum_norm, linewidth=2.0, label='$\sigma^-H^+$ LT')
##    plt.plot(RCEnergies, RCflipSpectrum_norm, 'k:', label='$\sigma^-H^-$')
    plt.plot(energies, XMCD, linewidth=2.0, label='$\sigma^+-\sigma^-$ x4 in $H^+$, LT')
    plt.plot(energies, XMCDflip, linewidth=2.0, label='$\sigma^+-\sigma^-$ x4 in $H^-$, LT')
    plt.plot(energies, XMCD_RT, 'k--', linewidth=2.0, label='$\sigma^+-\sigma^-$ x4 in $H^+$, RT')
    
    legend = plt.legend(loc='upper right', fontsize=16, shadow=False)
##    title = plt.title("Mn L XMCD spectrum of ex-situ STO|LMO(7) at different temperatures")
