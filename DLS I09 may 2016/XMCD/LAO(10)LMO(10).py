...
smooth = .03
FILE_LC = "../../Data/i09-75747.nxs"
FILE_LCflip = "../../Data/i09-75754.nxs"
FILE_RC = "../../Data/i09-75748.nxs"
FILE_RCflip = "../../Data/i09-75755.nxs"
...
def getBackground(energies, spectrum):
    ...
    points = [1, 35, 80, 155, 190, 275, 283]
    B = 1
    answ = "n"
...
if __name__ == '__main__':
    ...
    plt.plot(LCEnergies, LCSpectrum)
    plt.plot(LCEnergies, LCNorm)
    plt.plot(RCEnergies, RCSpectrum)
    plt.plot(RCEnergies, RCNorm)
    plt.show()
##    plt.plot(energies, LCSpectrum_norm, label='$\sigma^+H^+$')
##    plt.plot(RCEnergies, LCflipSpectrum_norm, 'k--',label='$\sigma^+H^-$')
##    plt.plot(energies, RCSpectrum_norm, label='$\sigma^-H^+$')
##    plt.plot(RCEnergies, RCflipSpectrum_norm, 'k:', label='$\sigma^-H^-$')
    plt.plot(energies, [0 for e in energies])
    plt.plot(energies, XMCD, label='$\sigma^+-\sigma^-$ x4 in $H^+$')
##    plt.plot(RCEnergies, [(LCSpectrum[n]-s)*4 for n, s in enumerate(RCSpectrum)])
    plt.plot(energies, XMCDflip, 'k--', label='$\sigma^+-\sigma^-$ x4 in $H^-$')
    
    legend = plt.legend(loc='upper right', shadow=True)
    title = plt.title("Mn L XMCD spectrum of ex-situ STO|LMO(4)")
