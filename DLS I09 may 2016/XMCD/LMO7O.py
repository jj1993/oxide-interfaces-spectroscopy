...
smooth = .00001
FILE_LC = "../../Data/i09-75695.nxs"
FILE_LCflip = "../../Data/i09-75715.nxs"
FILE_RC = "../../Data/i09-75696.nxs"
FILE_RCflip = "../../Data/i09-75716.nxs"
FILE_LC_RT = "../../Data/i09-75682.nxs"
FILE_RC_RT = "../../Data/i09-75683.nxs"
...
##    m_o, m_o_err, m_s, m_s_err = getMagnetism(RCSpectrum_norm, LCSpectrum_norm, energies, reg)
    ...
    MAG = [(LCSpectrum_norm[n]-s)*4 for n, s in enumerate(LCflipSpectrum_norm)]
    MAGflip = [(RCSpectrum_norm[n]-s)*4 for n, s in enumerate(RCflipSpectrum_norm)]

    # Save data to CSV
    spectra = [LCSpectrum_norm, LCflipSpectrum_norm, RCSpectrum_norm, RCflipSpectrum_norm, LCSpectrum_RT_norm, RCSpectrum_RT_norm, XMCD, XMCDflip, XMCD_RT, MAG]
...
##    plt.show()
    plt.plot(energies, LCSpectrum_norm, linewidth=2.0, label='$\sigma^+H^+$')
##    plt.plot(RCEnergies, LCflipSpectrum_norm, 'k--',label='$\sigma^+H^-$')
    plt.plot(energies, RCSpectrum_norm, linewidth=2.0, label='$\sigma^-H^+$')
##    plt.plot(RCEnergies, RCflipSpectrum_norm, 'k:', label='$\sigma^-H^-$')
    plt.plot(energies, XMCD, linewidth=2.0, label='$\sigma^+-\sigma^-$ x4 in $H^+$')
    plt.plot(energies, MAG, linewidth=2.0, label='$H^+-H^-$ x4 in $\sigma^+$')
    plt.plot(energies, MAGflip, linewidth=2.0, label='$H^+-H^-$ x4 in $\sigma^-$')
##    plt.plot(energies, XMCDflip, 'k--', label='$\sigma^+-\sigma^-$ x4 in $H^-$, LT')
##    plt.plot(energies, XMCD_RT, label='$\sigma^+-\sigma^-$ x4 in $H^+$, RT')
    
    legend = plt.legend(loc='upper right', shadow=True, fontsize=16)
##    title = plt.title("O K XMCD spectrum of ex-situ STO|LMO(7) at different temperatures")
