...
smooth = .005
FILE_LH = "../../Data/i09-75583.nxs"
FILE_LV = "../../Data/i09-75584.nxs"
...
def getBackground(energies, spectrum, points):
    ...
    try:
        for e in energies:
            ...
            n = diff/(math.exp((energies[points[2]]-e)/B)+1)
            m = diff/(math.exp((energies[points[4]]-e)/B)+1)
            step.append(n+m)
    except:
        diff = .5*(end - start)
        global diff
...
##        print points
    ...
    return [s+step[n] for n, s in enumerate(norm)], spectNorm
if __name__ == '__main__':
    ...
    print "============"
    points = [1, 120, 200, 250, 320, 410, 445]
...
##    plt.plot(LVEnergies, LHNorm)
    ...
    title = plt.title("Ti L XLD spectrum of in-situ STO|LMO(4)")
