...
smooth = .001
FILE_LH = "../../Data/i09-75667.nxs"
FILE_LV = "../../Data/i09-75668.nxs"
...
def getBackground(energies, spectrum):
    ...
    last = len(energies)
    points = [1, 110, 170, 240, 278, 405, 444]
    B = 1
    answ = "n"
...
##    LVSpectrum_norm = interpolate.splev(energies, splineRep, der=0)
    ...
    for n, s in enumerate(spectra):
        save(LHEnergies, s, str(n+1))

    # Plot data
    plt.plot(LHEnergies, [0 for e in LHEnergies])
##    plt.plot(LVEnergies, LHSpectrum)
##    plt.plot(LVEnergies, LHNorm)
    plt.plot(LHEnergies, LHSpectrum_norm, label='LH')
    plt.plot(LVEnergies, LVSpectrum_norm, label='LV')
    plt.plot(LVEnergies, XLD, label='XLD, LH - LV')
    
    legend = plt.legend(loc='upper right', shadow=True)
    title = plt.title("Ti L XLD spectrum of in-situ STO|LMO(2)")
